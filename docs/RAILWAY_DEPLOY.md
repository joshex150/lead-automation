...
| `cannot create a new collection, already using 501 collections of 500` | Your MongoDB is a full Atlas free cluster (M0 caps the whole cluster at 500 collections). The app needs only 5, so the cluster is full from other use. Point `MONGODB_URI` at a database with room: add Railway's MongoDB plugin (no collection limit) and use `${{MongoDB.MONGO_URL}}`, or create a fresh Atlas cluster, or drop unused collections on the existing one. |
| Dashboard shows "Can't reach the API" | If the message under it mentions collections or a Mongo error, see the row above, the API is reachable but the database is rejecting writes. Otherwise check `NEXT_PUBLIC_API_URL` build arg + API `DASHBOARD_ORIGIN` (CORS) + `NEXT_PUBLIC_API_KEY` matches `API_KEY`. |
